	...
	const point = () => {
		return subNav.map((l) => {
			return React.createElement(
				"div",
				{ className: Shared.scrollList + " pos" + l.id, key: l.id },
				<div className={Shared.image}>
					<img src={l.imageNav} alt={l.categorie} />
				</div>,
...
export function LoadSub() {
	...
	return <div className={Shared.load}>{repeat()}</div>;
}
export function LoadList({ test }) {
	return (
		<div className={Shared.boxing}>
			<div className={Shared.cirle}>{test}</div>
