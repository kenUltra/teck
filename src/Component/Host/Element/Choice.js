...
class Choice extends React.Component {
	...
	leftfilterData() {
		const data = this.state.startData.map((thing) => {
			return <LeftFilter key={thing.id} name={this.makeUpperCaseOne(thing.filter)} />;
		});
		return data;
	}
	rightfilterData() {
		const data = this.state.activityData.map((others) => {
			return <Rightfilter key={others.id} activities={this.makeUpperCaseOne(others.act)} />;
		});
		return data;
	}
	render() {
